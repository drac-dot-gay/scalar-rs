...
use surrealdb::types::{Array, Number, Object, SerializationError, Value};

use crate::serde_wrapper::WrappedError;
...
impl serde::de::Error for Error {
    ...
    where
        T: Display,
    {
        Self::serialization(msg.to_string(), Some(SerializationError::Deserialization))
    }
}

#[repr(transparent)]
pub(crate) struct WrappedValue(pub(crate) Value);
#[repr(transparent)]
struct WrappedObject(Object);
        ...
        Number::Decimal(decimal) => {
            if let Some(v) = decimal.to_i128() {
                ...
                Err(serde::de::Error::custom(
                    "decimal value not in range of an i128",
                ))
            }
        }
    }
...
impl<'de> serde::Deserializer<'de> for WrappedObject {
    type Error = Error;
    ...
    {
        let len = self.0.len();
        let mut deserializer = MapDeserializer::new(self.0);
...
impl<'de> serde::Deserializer<'de> for WrappedValue {
    type Error = Error;
        ...
        V: Visitor<'de>,
    {
        match self.0 {
            ...
            Value::Object(v) => WrappedObject(v).deserialize_any(visitor),
    ...
    {
        match self.0 {
            Value::Object(value) => WrappedObject(value).deserialize_enum(name, variants, visitor),
            Value::String(variant) => visitor.visit_enum(EnumDeserializer {
                variant,
                value: None,
            }),
            other => Err(serde::de::Error::invalid_type(
                WrappedValue(other).unexpected(),
                &"string or map",
            )),
        }
    }
...
impl<'de> IntoDeserializer<'de, Error> for WrappedValue {
...
impl<'de> VariantAccess<'de> for VariantDeserializer {
    ...
    fn unit_variant(self) -> Result<(), Error> {
        match self.value {
            Some(value) => Deserialize::deserialize(WrappedValue(value)),
            None => Ok(()),
        }
    }
    ...
    {
        match self.value {
            Some(value) => seed.deserialize(WrappedValue(value)),
    ...
    {
        match self.value {
            Some(Value::Object(v)) => WrappedObject(v).deserialize_any(visitor),
            Some(other) => Err(serde::de::Error::invalid_type(
                WrappedValue(other).unexpected(),
                &"struct variant",
            )),
...
impl<'de> SeqAccess<'de> for SeqDeserializer {
        ...
        T: DeserializeSeed<'de>,
    {
        match self.iter.next() {
            Some(value) => seed.deserialize(WrappedValue(value)).map(Some),
            None => Ok(None),
        }
    }
